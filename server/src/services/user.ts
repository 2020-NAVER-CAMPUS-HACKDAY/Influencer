...
export default class UserService {
  ...
  public async addWeight(productNo: number, weight: number): Promise<any> {
    const selectProduct = (productNo: number, weight: number): Promise<any> => {
    ...
    try {
      if (exist) {
        users.like[wholeCategoryId[0]].likeList = users.like[
          wholeCategoryId[0]
        ].likeList.filter((l: number) => l !== productNo);
  ...
  public async selectLikeList(page: number): Promise<any> {
    ...
    try {
      ...
      for (const categoryId of Object.keys(users.like)) {
        if (users.like[categoryId].likeList.length < 1) {
          ...
          for (const like of users.like[categoryId].likeList.slice(
            page * 10,
            page * 10 + 10,
          )) {
  ...
  public async recommendItem(page: number): Promise<IProductDTO[]> {
    ...
    const addRemainder = async (
      remainder: number,
      result: Array<IProductDTO>,
    ) => {
      const filtering = result.map((r: any) => r.productNo);
      const productRecord = await this.productModel
        .find()
        .where('productNo')
        .nin(filtering)
        .select(
          '-_id name category productNo salePrice productImages productInfoProvidedNoticeView',
        )
        .limit(remainder);
      const products = productRecord.map((record) => record.toObject());
    ...
    try {
      userRecord.forEach((user: IUser) =>
        user.prefer.sort((a: Prefer, b: Prefer) => b.rating - a.rating),
      );

      const documents = userRecord.map((user: IUser) => ({
        id: user.userName,
        content: user.prefer,
      }));

      recommender.train(documents);
      const collaborators = recommender.getSimilarDocuments(
        config.personaName,
        0,
        10,
      );
      collaborators.sort(
        (first: RecommenderResult, second: RecommenderResult) =>
          second.score - first.score,
      );

      let result: Array<IProductDTO> = [];
      if (collaborators.length <= page) {
        return await addRemainder(config.pagination, result);
      }

      const similarData = collaborators[page];
      const similarRecord = await this.userModel
        .findOne()
        .where('userName')
        .equals(similarData.id);
      ...
      for (const preference of similarRecord.prefer) {
        ...
        result.push(productRecord.toObject());
      }
  ...
  public async selectUserLikeList(): Promise<{ [index: string]: number[] }> {
    const userLikeRecord = await this.userModel.findOne({
      userName: config.personaName,
    });
    try {
      ...
      for (let categoryId of Object.keys(users.like)) {
        if (users.like[categoryId].likeList.length < 1) {
          ...
          result[users.like[categoryId].categoryName] =
            users.like[categoryId].likeList;
        }
      }
      return result;
    } catch (e) {
  ...
  public async getProductListSortedByModDate(
    idArray: string[],
  ): Promise<{ products: ProductVerGridView[] }> {
    try {
      const productArrayRecord = await this.productModel
        .find()
        .in('_id', idArray)
        .sort('modDate')
        .limit(4);
      ...
      const fetchedProducts = productArrayRecord.map((record) =>
        record.toObject(),
      );

      const products: ProductVerGridView[] = fetchedProducts.map(
        (product: FetchProductForGridView) => {
          return {
  ...
  public async selectLikeListForGridView(): Promise<{
    [index: string]: ProductVerGridView[];
  }> {
    try {
      const userLikeList: {
        [index: string]: number[];
      } = await this.selectUserLikeList();
      ...
      for (let category of Object.keys(userLikeList)) {
        const CategoryLikeProductList = await this.getProductListSortedByModDate(
          userLikeList[category].map((likeProductId) =>
            likeProductId.toString(),
          ),
        );
